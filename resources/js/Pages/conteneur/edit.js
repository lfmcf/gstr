...
export default function edit(props) {
    ...
    const { data, setData, post, processing, errors, clearErrors, reset } = useForm({
        id: cn.id,
        nom: cn.nom,
        date: moment(cn.date).format('YYYY-MM-DD HH:mm:ss'),
        produit: cn.product,
            ...
            <div style={{ marginTop: '20px' }}>
                <form onSubmit={handleSubmit}>
                    <Grid container spacing={3}>
                        <Grid item md={6}>
                            <TextField variant="outlined" fullWidth size='small' value={data.nom} label="Nom" name='nom' onChange={handleChange} error={errors.nom ? true : false} />
                        </Grid>
                        <Grid item md={6}>
                        ...
                        {data.produit.map((element, index) => (
                            <fieldset key={index} style={{ padding: '20px 10px', marginTop: '10px' }}>
                                ...
                                <div>
                                    ...
                                    <Grid container spacing={3}>
                                        <Grid item md={6}>
                                            <TextField variant='outlined' label="Nom produit" value={element.nom} name='nom' size='small' fullWidth onChange={(e) => handleProduitChange(index, e)} />
                                        </Grid>
                                        <Grid item md={6}>
                    ...
                    <Grid container spacing={3}>
                        <Grid item md={12}>
                            <TextareaAutosize minRows={3} style={{ width: '98%', padding: '8px 12px', borderRadius: '8px' }} value={data.description} name='description' onChange={handleChange} placeholder="Description" />
                        </Grid>
                    </Grid>
